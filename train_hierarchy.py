"""
Hierarchical variant of train.py
"""
import heapq
import logging
import os
...
task = "multiclass"
val_metrics = {
    "val/acc1": hierarchy.FineGrainedAccuracy(top_k=1),
    "val/acc5": hierarchy.FineGrainedAccuracy(top_k=5),
    "val/cross-entropy": hierarchy.FineGrainedCrossEntropy(),
}
val_metrics = torchmetrics.MetricCollection(val_metrics).to(device)
train_metrics = {
    "train/acc1": hierarchy.FineGrainedAccuracy(top_k=1),
    "train/acc5": hierarchy.FineGrainedAccuracy(top_k=5),
    "train/cross-entropy": hierarchy.FineGrainedCrossEntropy(),
}
...
def build_dataloader(*, train):
    ...
    dataset = hierarchy.ImageFolder(dataset_path, transform)
    # Don't know the number of classes until we load the dataset
    global num_classes
...
def train():
    ...
    global step
    model.train()
    mixup = hierarchy.Mixup(alpha=mixup_alpha, num_classes=num_classes)
    ...
    for batch, (inputs, targets) in enumerate(train_dataloader):
        ...
        if batch % (log_every * gradient_accumulation_steps) == 0 and is_master:
            ...
            metrics = {
                **train_metrics.compute(),
                "train/lr": lr,
                "train/multitask-cross-entropy": loss.item(),
                "train/step": step,
...
if __name__ == "__main__":
    ...
    loss_fn = hierarchy.MultitaskCrossEntropy(
        label_smoothing=label_smoothing, coeffs=coeffs
    ).to(device)
